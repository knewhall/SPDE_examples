% bead-spring (heat equation like) model with pinned ends - deterministic
function [X,erg] = det_evol(n_beads,eigenvalue_order,Tend)
...
X = zeros(n_beads,nstepmax);
X(:,1) = x.*(1-x);   % initial conditions

for nstep = 2:nstepmax
    % two pinned beads at either end
    Lpx = k_sp*( X(3:end,nstep-1) - 2*X(2:end-1,nstep-1) + X(1:end-2,nstep-1) )/dx2;

    X(2:end-1,nstep) = X(2:end-1,nstep-1) ...
        + CN/n_beads*(Lpx + Vp(X(2:end-1,nstep-1)))*dt;
end
